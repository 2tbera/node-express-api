const tableQuery = {
    ...
    category_table : "CREATE TABLE IF NOT EXISTS  `" + process.env.NODE_PROJECT + "`.`category` (`id` VARCHAR(36) NOT NULL , `music_id` VARCHAR(36) NULL, `name` VARCHAR(100) NOT NULL , PRIMARY KEY (`id`, `music_id`), FOREIGN KEY (`music_id`) REFERENCES music(id)) ENGINE = InnoDB;",
    user_role_table_many_to_many : "CREATE TABLE IF NOT EXISTS  `" + process.env.NODE_PROJECT + "`.`user_role` (`id` VARCHAR(36) NOT NULL , `role_id` VARCHAR(36) NULL,`user_id` VARCHAR(36) NULL, PRIMARY KEY (`id`, `role_id`, `user_id`), FOREIGN KEY (`role_id`) REFERENCES role(`id`) ON DELETE CASCADE, FOREIGN KEY (`user_id`) REFERENCES user(`id`) ON DELETE CASCADE) ENGINE = InnoDB;",
    album_music_table_many_to_many: "CREATE TABLE IF NOT EXISTS  `" + process.env.NODE_PROJECT + "`.`user_album` (`id` VARCHAR(36) NOT NULL , `album_id` VARCHAR(36) NULL,`user_id` VARCHAR(36) NULL, PRIMARY KEY (`id`, `album_id`, `user_id`), FOREIGN KEY (`album_id`) REFERENCES album(`id`) ON DELETE CASCADE, FOREIGN KEY (`user_id`) REFERENCES user(`id`) ON DELETE CASCADE) ENGINE = InnoDB;",
