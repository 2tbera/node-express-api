const tableQuery = {
    ...
    music_table : "CREATE TABLE IF NOT EXISTS  `" + process.env.NODE_PROJECT + "`.`music` (`id` VARCHAR(36) NOT NULL , `user_id` VARCHAR(36) NOT NULL, `file` VARCHAR(100) NOT NULL , `category` VARCHAR(100) NOT NULL ,`name` VARCHAR(100) NOT NULL , `status` BOOLEAN NOT NULL DEFAULT TRUE , `createAt` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP , `updatedAt` TIMESTAMP on update CURRENT_TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY (`id`, `user_id`), FOREIGN KEY (`user_id`) REFERENCES user(id)) ENGINE = InnoDB;",
